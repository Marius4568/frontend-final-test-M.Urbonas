...
})({"dOhcw":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
...
_asyncFunctions.dashboardAsyncFunctions.fillSelectCountryElem(countrySelect);
//fill the visitors card by a random number between 5000 and 10000
document.querySelector(".visitors").textContent = Math.floor(Math.random() * 5001 + 5000);
//fill the total signups field
_asyncFunctions.dashboardAsyncFunctions.updateTotalSignups();
//fill the signup countries field
_asyncFunctions.dashboardAsyncFunctions.updateSignupCountries();
//fill the uncapitalized names field
_asyncFunctions.dashboardAsyncFunctions.updateUncapitalizedNames();
...
searchField.addEventListener("keyup", (ev)=>{
    if (ev.target.value.length > 0) {
        ...
        displayPeople(query);
    } else displayPeople("http://18.193.250.181:1337/api/people?&pagination[pageSize]=10&populate=country");
...
async function displayPeople(url) {
    ...
    <div class="initialsContactInfo">
      <div class="initials">${el.attributes.first_name[0] + el.attributes.last_name[0]}</div>
      <div class="contact-info">
        <p>${el.attributes.first_name + " " + el.attributes.last_name}</p>
        <span>${el.attributes.email}</span>
      </div>
    </div>
    <div class="country">${el.attributes.country.data ? el.attributes.country.data.attributes.country : "No Country"}</div>
  </div>`;
        });
    } catch (err) {
...
function buildPeopleSearchQuery(searchVal, countryID) {
    ...
    if (countryID) query += `&filters[country][id][$eq]=${countryID}`;
    if (searchVal) query += `&filters[$or][0][first_name][$containsi]=${searchVal}&filters[$or][1][last_name][$containsi]=${searchVal}`;
    return query;
}
...
},{"./asyncFunctions":"kjQ1G"}],"kjQ1G":[function(require,module,exports) {
...
},{}]},["dOhcw","ktkKI"], "ktkKI", "parcelRequire6f59")
